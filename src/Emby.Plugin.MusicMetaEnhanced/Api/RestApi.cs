...
namespace Emby.Plugin.MusicMetaEnhanced.Api;
...
/// origins instead.
/// </remarks>
[Route("/MusicMeta/SaveConfig", "POST")]
[Unauthenticated]
...
public class RestApi : IService, IRequiresRequest
{
    ...
    public async Task Post(SaveConfigRequest request)
    {
        ...
        Request.Response.Redirect("/emby/MusicMeta/FormPage?v=19");
    }
...
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"/><title>Music Meta Enhanced</title></head>
...
<form method="post" action="/emby/MusicMeta/SaveConfig">
...
<a href="/emby/MusicMeta/Status?v=19" target="_blank" rel="noopener" style="display:inline-block;vertical-align:top;padding:0.6em 1.2em;border-radius:4px;background:#777;color:#fff !important;font-size:14px;line-height:1.5;text-decoration:none !important;">查看当前配置与状态</a>
</div>
</form>
...
<div>
<a href="/emby/MusicMeta/Action?op=refresh-artists&amp;v=19" class="am-btn-green" style="display:inline-block;vertical-align:top;margin:0 0.5em 0.5em 0;padding:0.6em 1.2em;border-radius:4px;background:#4caf50;color:#fff !important;font-size:14px;line-height:1.5;text-decoration:none !important;">刷新全部艺人元数据</a>
<a href="/emby/MusicMeta/Action?op=refresh-artists&amp;replace=true&amp;v=19" style="display:inline-block;vertical-align:top;margin:0 0.5em 0.5em 0;padding:0.6em 1.2em;border-radius:4px;background:#00a4dc;color:#fff !important;font-size:14px;line-height:1.5;text-decoration:none !important;">刷新全部艺人（替换已有值）</a>
</div>
<p style="color:#666;font-size:13px;margin:0 0 0.6em;">按名字匹配补全缺失的简介 / Apple Music ID；「替换已有值」会同时覆盖已有简介并把外部 ID 清空重写。</p>

<div>
<a href="/emby/MusicMeta/Action?op=refresh-albums&amp;v=19" style="display:inline-block;vertical-align:top;margin:0 0.5em 0.5em 0;padding:0.6em 1.2em;border-radius:4px;background:#4caf50;color:#fff !important;font-size:14px;line-height:1.5;text-decoration:none !important;">刷新全部专辑元数据</a>
<a href="/emby/MusicMeta/Action?op=refresh-albums&amp;replace=true&amp;v=19" style="display:inline-block;vertical-align:top;margin:0 0.5em 0.5em 0;padding:0.6em 1.2em;border-radius:4px;background:#00a4dc;color:#fff !important;font-size:14px;line-height:1.5;text-decoration:none !important;">刷新全部专辑（替换已有值）</a>
</div>
<p style="color:#666;font-size:13px;margin:0 0 0.6em;">用当前 AlbumStorefront 区域补全专辑名 / 年份 / 流派 / Apple Music ID。</p>

<div>
<a href="/emby/MusicMeta/Action?op=test-netease&amp;v=19" style="display:inline-block;vertical-align:top;margin:0 0.5em 0.5em 0;padding:0.6em 1.2em;border-radius:4px;background:#777;color:#fff !important;font-size:14px;line-height:1.5;text-decoration:none !important;">测试网易云 API 连通性</a>
<a href="/emby/MusicMeta/Action?op=test-apple&amp;v=19" style="display:inline-block;vertical-align:top;margin:0 0.5em 0.5em 0;padding:0.6em 1.2em;border-radius:4px;background:#777;color:#fff !important;font-size:14px;line-height:1.5;text-decoration:none !important;">测试 Apple Music 连通性</a>
</div>
...
(function(){
  ...
  document.addEventListener('click', function(e){
    var a = e.target.closest ? e.target.closest('a[href*="/emby/MusicMeta/Action"]') : null;
    if(!a){ return; }
    e.preventDefault();
  ...
  var form = document.querySelector('form[action*="/emby/MusicMeta/SaveConfig"]');
  if(form){
    form.addEventListener('submit', function(e){
      ...
      fetch(form.getAttribute('action'), {method:'POST', body: body, headers:{'Content-Type':'application/x-www-form-urlencoded'}})
        .then(function(){ return fetch('/emby/MusicMeta/FormPage'); })
        .then(function(r){ return r.text(); })
        .then(function(html){ document.open(); document.write(html); document.close(); fit(); });
    });
  }
...
""";
    ...
    private async Task WriteHtmlAsync(string title, string bodyHtml)
    {
        ...
        var page = "<!DOCTYPE html><html><head><meta charset=\"utf-8\"/><title>" + HtmlEncode(title) +
                   ...
                   "</p><p><a href=\"/emby/MusicMeta/FormPage?v=19\" onclick=\"location.replace('/emby/MusicMeta/FormPage?v=19');return false;\">返回 Music Meta Enhanced 配置页</a></p></div>" +
